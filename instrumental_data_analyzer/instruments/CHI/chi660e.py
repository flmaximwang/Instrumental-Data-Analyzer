...
class chi660eEIS:
    ...
    def txt_to_csv(txt_path, csv_name):
        ...
        file_name = (
            txt_path.replace(".txt", ".csv")
            if not csv_name
            else os.path.join(os.path.dirname(txt_path), csv_name + ".csv")
        )
...
class chi660eCV(VoltammegramCollection):
    ...
    def from_exported_files(txt_paths, name="CV"):
        ...
        for path, sig_name in zip(path_list, name_list):
            ...
            my_df = pd.read_csv(io.StringIO(data))
            my_df.rename(
                columns={"Potential/V": "Potential (V)", " Current/A": "Current (A)"},
                inplace=True,
            )
            ...
            scan_direction = (
                "Positive" if my_df.loc[1, "Potential (V)"] > E_init else "Negative"
            )
            ...
            if len(my_df) <= 2:
                ...
                for i in range(1, len(my_df) - 1):
                    if (
                        my_df.loc[i, "Potential (V)"]
                        - my_df.loc[i - 1, "Potential (V)"]
                    ) * (
                        my_df.loc[i + 1, "Potential (V)"]
                        - my_df.loc[i, "Potential (V)"]
                    ) < 0:
                ...
                my_df.loc[len(my_df) - 1, "Segment"] = my_df.loc[
                    len(my_df) - 2, "Segment"
                ]
        ...
        return VoltammegramCollection(signals=my_data_list, name=name)
