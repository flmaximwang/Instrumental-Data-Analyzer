...
class ImpedanceSpectrum(Signal):
    ...
    def __init__(self, data: pd.DataFrame, name: str = "Default Impedance Spectrum"):
        ...
        # 检查 data 的列名是否符合要求
        if not all(
            [col in data.columns for col in ["Frequency (Hz)", "Z' (ohm)", 'Z" (ohm)']]
        ):
            raise ValueError(
                "The column names of the data should be 'Frequency (Hz)', 'Z' (ohm)', 'Z\" (ohm))', while the current column names are "
                + str(data.columns)
            )
        self.data = data
        self.name = name
        self.description_annotations = [
            DescAnno("Frequency", "Hz"),
            DescAnno("Z'", "\\Omega"),
            DescAnno("Z''", "\\Omega"),
        ]
        self.fmt = ".-"

    def plot_nyquist(self, ax: plt.Axes, color="C0", scale=1):
...
class ImpedanceSpectrumCollection(SignalCollection):
    ...
    def __init__(
        self,
        signals: list[ImpedanceSpectrum] = [],
        name="Default_ImpedanceSpectrum_collection",
    ) -> None:
        ...
        for i, sig_name in enumerate(self):
            ...
            if isinstance(self.colormap, str):
                if self.colormap == "default":
                    ...
                    sig.plot_nyquist(
                        ax,
                        color=plt.get_cmap(self.colormap)(
                            i / len(self) * (self.colormap_max - self.colormap_min)
                            + self.colormap_min
                        ),
                    )
            elif isinstance(self.colormap, list):
                sig.plot_nyquist(ax, color=self.colormap[i], scale=scale)
            else:
                sig.plot_nyquist(ax, color=f"C{i}", scale=scale)
    ...
    def plot_bode(
        self, freq_limit=None, mag_limit=None, phase_limit=None, direction="horizontal"
    ):
